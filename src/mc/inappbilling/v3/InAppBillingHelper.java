 ...
 * process in-app billing operations. It provides synchronous (blocking) and asynchronous (non-blocking) methods for many
 * common in-app billing operations, as well as automatic signature verification.
 * 
 * After instantiating, you must perform setup in order to start using the object. To perform setup, call the
 * {@link #startSetup} method and provide a listener; that listener will be notified when setup is complete, after which (and
 * not before) you may call other methods.
 * 
 * After setup is complete, you may query whether the user owns a given item or not by calling {@link #isOwned}, get all
 * items owned with {@link #getOwnedSkus}, get an item's price with {@link #getPrice}, amongst others (see documentation for
 * specific methods).
 * 
 * Please notice that the object will only have knowledge about owned items; it will not automatically have information (such
 * as price, description) for items that are not owned by the user, because the server will not automatically provide those.
 * In order to query information for an item that's not owned (such as to display the price to the user before a purchase),
 * you should first bring the item's sku to the object's knowledge by calling {@link #addSku} and then perform an inventory
 * refresh by calling {@link #refreshInventory()} or its corresponding asynchronous version {@link #refreshInventoryAsync}.
 * 
 * If you know the skus of all the items that you can possibly be interested in, you can call {@link #addSku} for those items
 * before {@link #startSetup}, and that way all the information about them will be available from the start, with no need to
 * refresh the inventory later.
 * 
 * When you are done with this object, don't forget to call {@link #dispose} to ensure proper cleanup. This object holds a
 * binding to the in-app billing service, which will leak unless you dispose of it correctly. If you created the object on an
 * Activity's onCreate method, then the recommended place to dispose of it is the Activity's onDestroy method.
 * 
 * A note about threading: When using this object from a background thread, you may call the blocking versions of methods;
 * when using from a UI thread, call only the asynchronous versions and handle the results via callbacks. Also, notice that
...
{
    private static final String LOGTAG = InAppBillingHelper.class.getSimpleName();
     ...
     *            Your application's public key, encoded in base64. This is used for verification of purchase signatures. You
     ...
     */
    public InAppBillingHelper(Context ctx, String base64PublicKey)
    {
    ...
    /**
     * Callback for setup process. This listener's {@link #onIabSetupFinished} method is called when the setup process is
     * complete.
     */
    public interface OnIabSetupFinishedListener
    {
         ...
         */
        public void onIabSetupFinished(InAppBillingResult result);
    }
        ...
        mServiceConn = new ServiceConnection()
        {
            ...
            public void onServiceConnected(ComponentName name, IBinder service)
            {
                ...
                if(mService != null)
                {
                    ...
                    try
                    {
                        ...
                        if(response != BILLING_RESPONSE_RESULT_OK)
                        {
                            if(listener != null)
                            {
                                listener.onIabSetupFinished(new InAppBillingResult(response, "Error checking for billing v3 support."));
                            }
                            return;
                        }
                    ...
                    if(listener != null)
                    {
                        listener.onIabSetupFinished(new InAppBillingResult(BILLING_RESPONSE_RESULT_OK, "Setup successful."));
                    }
                    return;
                }
                if(listener != null)
                {
                    ...
                    listener.onIabSetupFinished(new InAppBillingResult(IABHELPER_UNKNOWN_ERROR, "Cannot connect to billing service."));
                }
            }
        };
        return mContext.bindService(new Intent("com.android.vending.billing.InAppBillingService.BIND"), mServiceConn, Context.BIND_AUTO_CREATE);
    }

    /**
     * Dispose of object, releasing resources. It's very important to call this method when you are done with this object. It
     * will release any resources used by it such as service connections. Naturally, once the object is disposed of, it can't
     * be used again.
     */
    public void dispose()
    {
    ...
    public interface OnIabPurchaseFinishedListener
    {
         ...
         * specifies which item was purchased. If the purchase failed, the sku and extraData parameters may or may not be
         ...
         */
        public void onIabPurchaseFinished(InAppBillingResult result, Purchase info);
    }
    ...
    public void launchPurchaseFlow(Activity act, String sku, int requestCode, OnIabPurchaseFinishedListener listener, String extraData)
    {
        ...
        catch(RemoteException e)
        {
            ...
            result = new InAppBillingResult(IABHELPER_REMOTE_EXCEPTION, "Remote exception while starting purchase flow");
            if(listener != null)
                listener.onIabPurchaseFinished(result, null);
        }
    }
...
/**
    ...
    public boolean handleActivityResult(int requestCode, int resultCode, Intent data)
    {
        ...
        if(resultCode == Activity.RESULT_OK && responseCode == BILLING_RESPONSE_RESULT_OK)
        {
            ...
            if(purchaseData == null || dataSignature == null)
            {
                ...
                result = new InAppBillingResult(IABHELPER_UNKNOWN_ERROR, "IAB returned null purchaseData or dataSignature");
                if(mPurchaseListener != null)
                    mPurchaseListener.onIabPurchaseFinished(result, null);
                return true;
            }
            ...
            try
            {
                ...
                if(!Security.verifyPurchase(mSignatureBase64, purchaseData, dataSignature))
                {
                    ...
                    result = new InAppBillingResult(IABHELPER_VERIFICATION_FAILED, "Signature verification failed for sku " + sku);
                    if(mPurchaseListener != null)
            ...
            if(mPurchaseListener != null)
            {
                mPurchaseListener.onIabPurchaseFinished(new InAppBillingResult(BILLING_RESPONSE_RESULT_OK, "Success"), purchase);
            }
        }
     ...
     * @param moreSkus
     *            additional skus to query information on, regardless of ownership. Ignored if null or if querySkuDetails is
     *            false.
     * @throws InAppBillingException
     *             if a problem occurs while refreshing the inventory.
     */
    public Inventory queryInventory(boolean querySkuDetails, List<String> moreSkus) throws InAppBillingException
    {
        ...
        try
        {
            ...
            {
                throw new InAppBillingException(r, "Error refreshing inventory (querying owned items).");
            }
            ...
            if(querySkuDetails)
            {
                ...
                if(r != BILLING_RESPONSE_RESULT_OK)
                {
                    throw new InAppBillingException(r, "Error refreshing inventory (querying prices of items).");
                }
            }
            return inv;
        }
        ...
        {
            throw new InAppBillingException(IABHELPER_REMOTE_EXCEPTION, "Remote exception while refreshing inventory.", e);
        }
        catch(JSONException e)
        {
            throw new InAppBillingException(IABHELPER_BAD_RESPONSE, "Error parsing JSON response while refreshing inventory.", e);
        }
    }
    ...
    public interface QueryInventoryFinishedListener
    {
         ...
         *            The inventory.
         */
        public void onQueryInventoryFinished(InAppBillingResult result, Inventory inv);
    }

    /**
     * Asynchronous wrapper for inventory query. This will perform an inventory query as described in {@link #queryInventory}
     * , but will do so asynchronously and call back the specified listener upon completion. This method is safe to call from
     * a UI thread.
     * 
     * @param querySkuDetails
    ...
    public void queryInventoryAsync(final boolean querySkuDetails, final List<String> moreSkus, final QueryInventoryFinishedListener listener)
    {
        ...
        new Thread(new Runnable()
        {
            public void run()
            {
                InAppBillingResult result = new InAppBillingResult(BILLING_RESPONSE_RESULT_OK, "Inventory refresh successful.");
                Inventory inv = null;
                try
                {
                ...
                final InAppBillingResult result_f = result;
    ...
    /**
     * Consumes a given in-app product. Consuming can only be done on an item that's owned, and as a result of consumption,
     * the user will no longer own it. This method may block or take long to return. Do not call from the UI thread. For
     ...
     */
    void consume(Purchase itemInfo) throws InAppBillingException
    {
        ...
        try
        {
            ...
            if(token == null || token.equals(""))
            {
                ...
                throw new InAppBillingException(IABHELPER_MISSING_TOKEN, "PurchaseInfo is missing token for sku: " + sku + " " + itemInfo);
            }
            ...
            else
            {
                ...
                throw new InAppBillingException(response, "Error consuming sku " + sku);
            }
        }
        catch(RemoteException e)
        {
            throw new InAppBillingException(IABHELPER_REMOTE_EXCEPTION, "Remote exception while consuming. PurchaseInfo: " + itemInfo, e);
        }
    }
    ...
    public interface OnConsumeFinishedListener
    {
         ...
         */
        public void onConsumeFinished(Purchase purchase, InAppBillingResult result);
    }
    ...
    public interface OnConsumeMultiFinishedListener
    {
         ...
         */
        public void onConsumeMultiFinished(List<Purchase> purchases, List<InAppBillingResult> results);
    }

    /**
     * Asynchronous wrapper to item consumption. Works like {@link #consume}, but performs the consumption in the background
     * and notifies completion through the provided listener. This method is safe to call from a UI thread.
     * 
     * @param purchase
        ...
        (new Thread(new Runnable()
        {
            public void run()
            {
                ...
                for(Purchase purchase : purchases)
                {
                    ...
                    {
                        consume(purchase);
                        results.add(new InAppBillingResult(BILLING_RESPONSE_RESULT_OK, "Successful consume of sku " + purchase.getSku()));
                    }
                    catch(InAppBillingException ex)
                    {
    ...
    {
        Log.e(LOGTAG, "In-app billing error: " + msg);
    }

    void logWarn(String msg)
    {
        Log.w(LOGTAG, "In-app billing warning: " + msg);
    }
}
